...
bool_t bmi088_accel_init(void)
{
    ...
    BMI088_accel_read_single_reg(BMI088_ACC_CHIP_ID, res_p);
    ...
    // check the "who am I"
    if (res_p != BMI088_ACC_CHIP_ID_VALUE)
    {
    ...
    for (write_reg_num = 0; write_reg_num < BMI088_WRITE_ACCEL_REG_NUM; write_reg_num++)
    {
        ...
        BMI088_accel_read_single_reg(write_BMI088_accel_reg_data_error[write_reg_num][0], res_p);
        BMI088_delay_ms(BMI088_LONG_DELAY_TIME);
			//BMI088_delay_us(BMI088_COM_WAIT_SENSOR_TIME);

        if (res_p != write_BMI088_accel_reg_data_error[write_reg_num][1])
        {
...
bool_t bmi088_gyro_init(void)
{
    ...
    //check commiunication
    BMI088_gyro_read_single_reg(BMI088_GYRO_CHIP_ID, res_g);
    BMI088_delay_us(BMI088_COM_WAIT_SENSOR_TIME);
    BMI088_gyro_read_single_reg(BMI088_GYRO_CHIP_ID, res_g);
    BMI088_delay_us(BMI088_COM_WAIT_SENSOR_TIME);
    ...
    // check the "who am I"
    if (res_g != BMI088_GYRO_CHIP_ID_VALUE)
    {
        return BMI088_NO_SENSOR;
    }
    ...
    for (write_reg_num = 0; write_reg_num < BMI088_WRITE_GYRO_REG_NUM; write_reg_num++)
    {
        ...
        BMI088_gyro_read_single_reg(write_BMI088_gyro_reg_data_error[write_reg_num][0], res_g);
        BMI088_delay_us(BMI088_COM_WAIT_SENSOR_TIME);

        if (res_g != write_BMI088_gyro_reg_data_error[write_reg_num][1])
        {

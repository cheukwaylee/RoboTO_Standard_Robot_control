...
void gimbal_control_loop(void) {
	...
	standard_gimbal.y[0] = ins_correct_angle[2];			// angular position of motor 0x205 (Yaw of gimbal)
	standard_gimbal.y[1] = ins_correct_angle[0];			// angular position of motor 0x206 (Pitch of gimbal)
	standard_gimbal.y[2] = bmi088_real_data.gyro[2];	// angular velocity of motor 0x205
	standard_gimbal.y[3] = bmi088_real_data.gyro[0];	// angular velocity of motor 0x206
